...
// model imports used via services in AdminControllerUtils
...
public class AdminController {
    ...
    private void bindColumns() {
        ...
        setupTable(offeringTable, offeringData, colOfferingCourseOfferingId, colOfferingCourseId, colOfferingCourseName, colOfferingCredits, colOfferingInstructor, colOfferingSemesterId, colOfferingSchedule, colOfferingRoomId, colOfferingMaxCapacity, colOfferingRemaining);
    ...
    private void bindActions() {
        ...
        List<Button> reloadButtons = List.of(offeringReloadBtn, courseReloadBtn, userReloadBtn);
        List<Button> detailButtons = List.of(offeringDetailBtn, courseDetailBtn, userDetailBtn);
        List<Button> editButtons = List.of(offeringEditBtn, courseEditBtn, userEditBtn);
        for (Button btn : reloadButtons) 
            if (btn != null) btn.setOnAction(e -> handleReload(e));
        for (Button btn : detailButtons) 
            if (btn != null) btn.setOnAction(e -> handleViewDetail(e));
        for (Button btn : editButtons)
            if (btn != null) btn.setOnAction(e -> handleEdit(e));
    }
    ...
    private void handleAdd(ActionEvent event) {
        try {
            ...
            if (src == offeringAddBtn) navigationManager.showCourseOfferingAddForm();
            else if (src == courseAddBtn) navigationManager.showCourseAddForm();
            else if (src == userAddBtn) navigationManager.showUserAddForm();
            ...
            if (event.getSource() == offeringEditBtn) {
                ...
                AdminControllerHelper.requireSelectedAndFetch(
                    ...
                    (User fullUser) -> {
                        ...
                        // Refresh users after edit form opens (form itself may perform final refresh)
                        loadUserData();
                    }
                );
            }
